class BattleshipGame:
    ...
    # This method runs the game.  It first lets the two players position their boats then goes into a loop,
    # in which players take alternate turns until there a winner emerges.
    def play(self):
        ...
        # Next, begin the game. Repeat the game-play loop as long as there is no winner.
        input("The boats are ready... it's time to play.  Press enter to begin!")
        winner = False
        first_players_turn = True
        while not winner:
            # Takes a turn for the next player.
            ...
            # Swaps the turn so that the other player goes on the next iteration.
...
# Boat: A cell that has a boat.  Represented as " B".
# Empty: A cell that has no boat. Represented as " _".
# Hit: A cell that was a boat, but one that was hit by an attack. Represented as "X"
# Miss: A cell that was attacked but has no boat.  Represented as "_O"
class Board:

    # Initializes the board to a 10x10 grid of empty cells
    def __init__(self):
        # The grid
        ...
        # can simply count hits as they happen until it reaches 17.
        self.hit_count = 0
    ...
    # Adds a boat to the board.  Returns False if the boat can't be legally positioned at the requested location.
    ...
    def add_boat(self, boat):
        ...
        # The function would have returned False if the board doesn'tss fit.  That means the position is valid.
...
class Boat:
    # Initialize the boat with a label (e.g., Destroyer) and a size (e.g., 3).  The position will be set later and will
    # be undefined even after the initialization method.s
...
class HumanPlayer:
    ...
    def position_boat(self, boat):
        # Shows the board as it exists before this boat is positioned.
        print(self.board)
        print("You need to position a", boat.label, "of length", boat.size, "on the board above.")
        # Asks the user to say if the boat is horizontal or vertical.
        orientation = None
